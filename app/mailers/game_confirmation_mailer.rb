class GameConfirmationMailer < ActionMailer::Base
  ...
  #
  def friendly_game_confirmation_ask(game)
    @game = game
  ...
  def friendly_game_confirmation(game, email_destination_team)
    friendly_game_new_status(game, email_destination_team, 'confirm')
  end

  def friendly_game_cancellation(game, email_destination_team)
